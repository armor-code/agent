...
- The agent interacts with AWS S3 via HTTPS using a **pre-signed URL** (issued by the ArmorCode server).
- The pre-signed URL is valid for 10 minutes.
- The S3 bucket is **private and hosted within ArmorCode's AWS account**.
...
## Setting Up the Agent Using the Python Script
